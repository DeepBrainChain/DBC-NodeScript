...
const args = minimist(process.argv.slice(2), { string: ['key', 'wallet' , 'length'] });
// 链上交互
...
export const confirmRent = async (wallet, length) => {
  await GetApi();
...
confirmRent(args["wallet"], args["length"]).catch((error) => console.log(error.message))
 ...
 * 1. 执行JS
 * 
 * node confirmRentBatch.js --key 0x11111 --wallet 5FL.... --length 6
 * key 为 种子
 * wallet 为钱包地址
 * length 为提交租用的机器个数
 */